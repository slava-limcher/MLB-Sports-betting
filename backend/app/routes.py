...
async def get_todays_games(day: str | None = None) -> Dict[str, Any]:
    """
    Fetch MLB games from balldontlie for a given day (defaults to today).
    Used by the venue operator to pick which game a room tracks.
    `day` is an ISO date string (YYYY-MM-DD); invalid/empty falls back to today.
    """
    target = (day or "").strip() or date.today().isoformat()
    games = await bdl_client.get_games(dates=[target])
    return {"games": games, "date": target}
